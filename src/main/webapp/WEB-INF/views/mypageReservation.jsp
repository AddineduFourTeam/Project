                        ...
                        <div class="rv_info">
                            ...
                            <div class="info_con">
                                ...
                                <ul>
                                    ...
                                    <li>
                                        <p class="tit">총 금액</p>
                                        <p class="txt price" id="revPrice"><fmt:formatNumber value="${resv.rvPrice}" pattern="#,###"/>원</p>
                                    </li>
                                </ul>
                                <div class="btn_wrap">
                                    <a href="javascript:void(0)" class="update" id="rvidx${resv.rvIdx}">예약 수정</a>
    ...
    $(".btn_wrap .update").click(function(){
        ...
        $.ajax({
            ...
            success: function(data) {
                console.log(data.rvPrice);
                $(".rs_year").text((data.rvUseDate)+"년 ("+ data.rvUseYearDate+")");
                $(".rs_total_price").text(data.rvPrice);
                $(".rs_feet").text(`${"${data.rvFeet}"}평`);

                setOptions(data.rvOptionsData); // rvOptionsData를 처리하는 함수, 구현 필요

                function setOptions(optionsData) {
                        if (data.rvOptionSeeding === 'Y') $("#op1").prop('checked', true);
                        if (data.rvOptionPlow === 'Y') $("#op2").prop('checked', true);
                        if (data.rvOptionWatering === 'Y') $("#op3").prop('checked', true);
                        if (data.rvOptionCompost === 'Y') $("#op4").prop('checked', true);
                }
                let arr = data.rvUseYearDate.split(",");
                ...
                $(".feet span").text(data.rvFeet);

                //console.log(data);
                console.log( $("#rvMemIdx").val(data.rvMemIdx));
            },
